// --------------------------------------------------------------------------------------------------------------------
// <copyright file="Queue.cs" company="The original author or authors.">
...
namespace Spring.Messaging.Amqp.Core
{
    ...
    public class Queue
    {
        ...
        /// <summary>Initializes a new instance of the <see cref="Queue"/> class, given a name and durability flag. The queue is non-exclusive and non auto-delete.</summary>
        /// <param name="name">The name of the queue.</param>
        /// <param name="durable">The durable. True if we are declaring a durable queue (the queue will survive a server restart).</param>
        public Queue(string name, bool durable) : this(name, durable, false, false, null) { }

        /// <summary>Initializes a new instance of the <see cref="Queue"/> class, given a name, durability, exclusive and auto-delete flags.</summary>
        /// <param name="name">The name of the queue.</param>
        /// <param name="durable">The durable. True if we are declaring a durable queue (the queue will survive a server restart).</param>
        ...
        /// <summary>Initializes a new instance of the <see cref="Queue"/> class, given a name, durability flag, and auto-delete flag, and arguments.</summary>
        /// <param name="name">The name of the queue.</param>
        /// <param name="durable">The durable. True if we are declaring a durable queue (the queue will survive a server restart).</param>
        /// <param name="exclusive">The exclusive. True if we are declaring an exclusive queue (the queue will only be used by the declarer's connection).</param>
        /// <param name="autoDelete">The auto delete. True if the server should delete the queue when it is no longer in use.</param>
        ...
        /// </summary>
        public bool Exclusive { get { return this.exclusive; } }
        ...
        /// </returns>
        public override string ToString() { return string.Format("Name: {0}, Durable: {1}, AutoDelete: {2}, Exclusive: {3}, Arguments: {4}", this.name, this.durable, this.autoDelete, this.exclusive, this.arguments); }
    }
}
