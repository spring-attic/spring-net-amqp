// --------------------------------------------------------------------------------------------------------------------
// <copyright file="IBlockingQueue.cs" company="The original author or authors.">
...
namespace Spring.Threading.Collections.Generic
{
    ...
    /// <remarks><para><see cref="IBlockingQueue{T}"/> methods come in four forms, with different ways
    /// of handling operations that cannot be satisfied immediately, but may be
    /// satisfied at some point in the future:
    /// <list type="bullet">
    /// <item>
    /// one throws an exception,</item>
    /// <item>the second returns a special value (either <c>default(T)</c> or <see langword="false"/>, depending on the operation), </item>
    /// <item>the third blocks the current thread indefinitely until the operation can succeed,
    /// and </item>
    /// <item>the fourth blocks for only a given maximum time limit before giving
    /// up.</item>
    /// </list>
    /// </para>
    /// <para>A <see cref="IBlockingQueue{T}"/> may be capacity bounded. At any given
    ...
    /// reports a remaining capacity of <see cref="System.Int32.MaxValue"/>.</para>
    /// <para><see cref="IBlockingQueue{T}"/> implementations are designed to be used
    /// primarily for producer-consumer queues, but additionally support
    /// the <see cref="ICollection{T}"/> interface.  So, for example, it is
    /// possible to remove an arbitrary element from a queue using<see cref="ICollection{T}.Remove(T)"/>. 
    /// However, such operations are in general<b>not</b> performed very efficiently, and are intended for only
    /// occasional use, such as when a queued message is cancelled.</para>
    /// <para>A <see cref="IBlockingQueue{T}"/> does <b>not</b> intrinsically support
    /// any kind of 'close' or 'shutdown' operation to
    /// indicate that no more items will be added.  The needs and usage of
    /// such features tend to be implementation-dependent. For example, a
    /// common tactic is for producers to insert special<b>end-of-stream</b> or <b>poison</b> objects, that are
    /// interpreted accordingly when taken by consumers.</para>
    /// <para>A <see cref="IBlockingQueue{T}"/> can safely be used with multiple
    ...
    public interface IBlockingQueue<T> : IQueue<T>
    {
        ...
        /// <exception cref="System.ArgumentException">If some property of the supplied <paramref name="element"/> prevents
        ...
        /// <exception cref="System.ArgumentNullException">If the specified element is <see langword="null"/> and this queue 
        /// does not permit <c>null</c> elements.</exception>
        /// <exception cref="System.ArgumentException">If some property of the supplied <paramref name="element"/> prevents
        /// it from being added to this queue.</exception>
        ...
        /// exception is thrown.  Attempts to drain a queue to itself result in<see cref="System.ArgumentException"/>. Further, the behavior of
        /// this operation is undefined if the specified collection is
        /// modified while the operation is in progress.</remarks>
        ...
        /// <exception cref="System.InvalidCastException">If the class of the supplied <paramref name="collection"/> prevents it
        /// from being used for the elemetns from the queue.</exception>
        ...
        /// thrown.  Attempts to drain a queue to itself result in<see cref="System.ArgumentException"/>. Further, the behavior of
        /// this operation is undefined if the specified collection is
        /// modified while the operation is in progress.</remarks>
        /// <param name="collection">the collection to transfer elements into</param>
