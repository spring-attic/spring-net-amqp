// --------------------------------------------------------------------------------------------------------------------
// <copyright file="IQueue.cs" company="The original author or authors.">
...
namespace Spring.Collections.Generic
{
    ...
    /// <remarks><para>Besides basic <see cref="ICollection{T}"/> operations, queues provide 
    /// additional insertion, extraction, and inspection operations. </para>
    /// <para>Each of these methods exists in two forms: one throws an exception if 
    ...
    /// implementations, insert operations cannot fail. </para>
    /// <para>Queues typically, but do not necessarily, order elements in a FIFO 
    ...
    /// implementation must specify its ordering properties. </para>
    /// <para>The <see cref="Offer"/> method inserts an element if possible, 
    /// otherwise returning <c>false</c>. This differs from the <see cref="ICollection{T}.Add"/> method, which can fail to add an 
    ...
    /// occurrence, for example, in fixed-capacity (or "bounded") queues. </para>
    /// <para>The <see cref="Remove"/> and <see cref="Poll"/> methods remove and 
    ...
    /// from implementation to implementation. The <see cref="Remove"/> and Poll<see cref="Poll"/> methods differ only in their behavior when the queue 
    /// is empty: the <see cref="Remove"/> method throws an exception, while 
    /// the <see cref="Poll"/> method returns <c>false</c>. </para>
    /// <para>The <see cref="Element"/> and <see cref="Peek"/> methods return, but do 
    /// not remove, the head of the queue. </para>
    /// <para>The <see cref="IQueue{T}"/> interface does not define the blocking queue 
    /// methods, which are common in concurrent programming. </para>
    /// <para><see cref="IQueue{T}"/> implementations generally do not define 
